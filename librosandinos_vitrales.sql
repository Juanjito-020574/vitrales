...
-- Tiempo de generación: 24-07-2025 a las 11:03:04
...
--
-- Base de datos: `librosandinos_vitrales`
--
CREATE DATABASE IF NOT EXISTS `librosandinos_vitrales` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='VIEW';

--
-- Volcado de datos para la tabla `cdm`
--
...
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='campo=&titulo=&visible=&nuevo=&edit=&select=$this->funSelect(''TABLA'',''CAMPO_id,CAMPO_ver'')&predet=&attr=&clase=algn_left|algn_number&thwdth=';
...
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='campo=&titulo=&visible=&nuevo=&edit=&select=$this->funSelect(''TABLA'',''CAMPO_id,CAMPO_ver'')&predet=&attr=&clase=algn_left|algn_number&thwdth=';
...
) ENGINE=InnoDB AUTO_INCREMENT=204 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='campo=&titulo=&visible=&nuevo=&edit=&select=$this->funSelect(''TABLA'',''CAMPO_id,CAMPO_ver'')&predet=&attr=&clase=algn_left|algn_number&thwdth=';
...
--
-- Disparadores `cotizacion_detalle`
--
...
--
-- Estructura Stand-in para la vista `cotizacion_detalle_materiales`
-- (Véase abajo para la vista actual)
--
DROP VIEW IF EXISTS `cotizacion_detalle_materiales`;
CREATE TABLE IF NOT EXISTS `cotizacion_detalle_materiales` (
`cotizacion_id` int(11)
,`materiales_id` varchar(30)
,`nombre` varchar(150)
,`formula` mediumtext
,`unidad` varchar(10)
);
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='campo=&titulo=&visible=&nuevo=&edit=&select=$this->funSelect(''TABLA'',''CAMPO_id,CAMPO_ver'')&predet=&attr=&clase=algn_left|algn_number&thwdth=';
...
--
-- Estructura Stand-in para la vista `materiales_color`
-- (Véase abajo para la vista actual)
--
DROP VIEW IF EXISTS `materiales_color`;
CREATE TABLE IF NOT EXISTS `materiales_color` (
`materiales_id` varchar(30)
,`nombre` varchar(150)
,`color` varchar(50)
);
...
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='campo=&titulo=&visible=&nuevo=&edit=&select=$this->funSelect(''TABLA'',''CAMPO_id,CAMPO_ver'')&predet=&attr=&clase=algn_left|algn_number&thwdth=';
...
--
-- Estructura de tabla para la tabla `tipologia_materiales`
--
...
) ENGINE=InnoDB AUTO_INCREMENT=166 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='campo=&titulo=&visible=&nuevo=&edit=&select=$this->funSelect(''TABLA'',''CAMPO_id,CAMPO_ver'')&predet=&attr=&clase=algn_left|algn_number&thwdth=';

--
-- Volcado de datos para la tabla `tipologia_materiales`
--
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='campo=&titulo=&visible=&nuevo=&edit=&select=$this->funSelect(''TABLA'',''CAMPO_id,CAMPO_ver'')&predet=&attr=&clase=algn_left|algn_number&thwdth=';
...
--
-- Estructura para la vista `cotizacion_detalle_materiales`
--
DROP TABLE IF EXISTS `cotizacion_detalle_materiales`;

DROP VIEW IF EXISTS `cotizacion_detalle_materiales`;
CREATE ALGORITHM=UNDEFINED DEFINER=`librosandinos`@`localhost` SQL SECURITY DEFINER VIEW `cotizacion_detalle_materiales`  AS SELECT `cotizacion_detalle`.`cotizacion_id` AS `cotizacion_id`, `tipologia_materiales`.`materiales_id` AS `materiales_id`, (select `materiales`.`nombre` from `materiales` where `materiales`.`materiales_id` = `tipologia_materiales`.`materiales_id`) AS `nombre`, concat('{"',group_concat(concat(replace(`tipologia_materiales`.`materiales_id`,'_',''),'_',replace(`cotizacion_detalle`.`codificacion`,'_',''),'_',`tipologia_materiales`.`ensamble`,'": {"dim":"',replace(replace(replace(replace(replace(replace(`tipologia_materiales`.`formula`,'$Dh',`tipologia`.`div_horizontal`),'$Dv',`tipologia`.`div_vertical`),'$c','0.01'),'$M',`tipologia`.`movil`),'$h',`cotizacion_detalle`.`alto`),'$A',`cotizacion_detalle`.`ancho`),'", "cant":"',`tipologia_materiales`.`cantidad`,'"} ') separator ', "'),'}') AS `formula`, (select `materiales`.`unidad` from `materiales` where `materiales`.`materiales_id` = `tipologia_materiales`.`materiales_id`) AS `unidad` FROM ((`tipologia` join `cotizacion_detalle` on(`tipologia`.`tipologia_id` = `cotizacion_detalle`.`tipologia_id`)) join `tipologia_materiales` on(`tipologia`.`tipologia_id` = `tipologia_materiales`.`tipologia_id`)) GROUP BY `cotizacion_detalle`.`cotizacion_id`, `tipologia_materiales`.`materiales_id` ORDER BY `cotizacion_detalle`.`cotizacion_id` ASC, `tipologia_materiales`.`materiales_id` ASC, `cotizacion_detalle`.`codificacion` ASC ;

-- --------------------------------------------------------

--
-- Estructura para la vista `materiales_color`
--
DROP TABLE IF EXISTS `materiales_color`;

DROP VIEW IF EXISTS `materiales_color`;
CREATE ALGORITHM=UNDEFINED DEFINER=`librosandinos`@`localhost` SQL SECURITY DEFINER VIEW `materiales_color`  AS SELECT `materiales`.`materiales_id` AS `materiales_id`, `materiales`.`nombre` AS `nombre`, `color`.`color` AS `color` FROM (`materiales` join `color`) WHERE `materiales`.`colores` regexp concat('.*',`color`.`color`,'.*') ;
...
ALTER TABLE `cotizacion`
  ADD CONSTRAINT `FK_cotizacion_clientes` FOREIGN KEY (`clientes_id`) REFERENCES `clientes` (`clientes_id`) ON UPDATE CASCADE;

--
-- Filtros para la tabla `cotizacion_detalle`
--
ALTER TABLE `cotizacion_detalle`
  ADD CONSTRAINT `FK_cotizacion_detalle_cotizacion` FOREIGN KEY (`cotizacion_id`) REFERENCES `cotizacion` (`cotizacion_id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_cotizacion_detalle_tipologia` FOREIGN KEY (`tipologia_id`) REFERENCES `tipologia` (`tipologia_id`) ON UPDATE CASCADE;

--
-- Filtros para la tabla `materiales`
--
ALTER TABLE `materiales`
  ADD CONSTRAINT `FK_materiales_familias` FOREIGN KEY (`familias_id`) REFERENCES `familias` (`familias_id`) ON UPDATE CASCADE;

--
-- Filtros para la tabla `precios_materiales`
--
ALTER TABLE `precios_materiales`
  ADD CONSTRAINT `FK_precios_materiales_color` FOREIGN KEY (`color`) REFERENCES `color` (`color`) ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_precios_materiales_materiales` FOREIGN KEY (`materiales_id`) REFERENCES `materiales` (`materiales_id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_precios_materiales_proveedores` FOREIGN KEY (`proveedores_id`) REFERENCES `proveedores` (`proveedores_id`) ON UPDATE CASCADE;

--
-- Filtros para la tabla `tipologia`
--
ALTER TABLE `tipologia`
  ADD CONSTRAINT `FK_tipologia_familias` FOREIGN KEY (`familias_id`) REFERENCES `familias` (`familias_id`) ON UPDATE CASCADE;

--
-- Filtros para la tabla `tipologia_materiales`
--
ALTER TABLE `tipologia_materiales`
  ADD CONSTRAINT `FK_tipologia_materiales_materiales` FOREIGN KEY (`materiales_id`) REFERENCES `materiales` (`materiales_id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_tipologia_materiales_tipologia` FOREIGN KEY (`tipologia_id`) REFERENCES `tipologia` (`tipologia_id`) ON UPDATE CASCADE;
COMMIT;
